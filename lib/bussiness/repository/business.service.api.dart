...
class BusinessAPI {
  ...
  Future<List<Business>> fetchBusines(String idClient) async {
    ...
    final response = await http.get(BusinessConstants.GETBUSINESS + idClient,
        headers: headers);
  ...
  List<TimeAttentionBusiness> parseTimeBusiness(String responseBody) {
    final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
    return parsed
        .map<TimeAttentionBusiness>(
            (json) => TimeAttentionBusiness.formJson(json))
        .toList();
  }

  Future<List<TimeAttentionBusiness>> fetchTimeBusines(
      String idBusiness) async {
    SharedPreferences _preferences = await SharedPreferences.getInstance();
    String token = _preferences.getString('Token');
    Map<String, String> headers = {"Authorization": "bearer" + token};
    final response = await http.get(
        BusinessConstants.GETTIMEATTENTION + idBusiness + "/horarioAtencion",
        headers: headers);
